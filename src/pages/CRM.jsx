...
import { CRM_LEADS, CRM_STAGES } from '../data/seed'
...
export default function CRM(){
  ...
  const convRate=((leads.filter(l=>l.stage==="Converted").length/leads.length)*100).toFixed(0)
  ...
  return <div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
      ...
      <KPI label="Avg Lead Value"  value={fmt(Math.round(totalPipe/leads.length))} sub="Per lead" accent={T.purple} icon="◈"/>
    </div>
      ...
      <div style={{display:"flex",gap:10,minWidth:860,paddingBottom:8}}>
        {CRM_STAGES.map(stage=>{
          ...
          return <div key={stage} style={{flex:1,minWidth:138}}
            ...
            onDrop={()=>{if(dragging){setLeads(ls=>ls.map(l=>l.name===dragging.name?{...l,stage}:l));setDragging(null)};setHoverCol(null)}}
            ...
            <div style={{background:hoverCol===stage?`${sc}06`:T.surface,
              ...
              {stageLeads.map((l,i)=>(
                <div key={i} draggable onDragStart={()=>setDragging(l)}
