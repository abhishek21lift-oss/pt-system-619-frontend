...
Activity: ${form.activity}, Goal: ${goalL[form.goal]}, Diet: ${dietL[form.diet]}
...
JSON format: {"tip":"one coaching insight","foods":["5 best Indian foods for goal"],"avoid":["3 foods to avoid"],"hydration":"water recommendation","supplement":"top 2 supplements"}`
  ...
  const iField={background:"rgba(255,255,255,0.04)",border:`1px solid ${T.border}`,borderRadius:9,
    ...
    <div style={{display:"grid",gridTemplateColumns:"370px 1fr",gap:16}}>
      ...
      <div>
        ...
        {loading&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",
          ...
          <div style={{width:48,height:48,borderRadius:"50%",border:`3px solid ${T.gold}28`,
            borderTop:`3px solid ${T.gold}`,animation:"spin 1s linear infinite"}}/>
          <p style={{color:T.gold,fontSize:13,fontWeight:700}}>Claude is building your plan…</p>
        </div>}
        {result&&<div>
          {err&&<p style={{color:T.orange,fontSize:11,marginBottom:12,background:`${T.orange}10`,
            border:`1px solid ${T.orange}20`,borderRadius:8,padding:"6px 12px"}}>{err}</p>}
          ...
          <Card style={{padding:20,marginBottom:12}}>
            ...
            {result.meals.map((m,i)=>(
              <div key={i} style={{display:"flex",alignItems:"flex-start",gap:14,
                padding:"11px 0",borderBottom:i<result.meals.length-1?`1px solid ${T.border}`:"none"}}>
