...
Include EXACTLY ${form.days} training days. Use Indian gym-friendly exercises.`
      ...
      setResult({
        program_name:`${form.days}-Day ${form.style.charAt(0).toUpperCase()+form.style.slice(1)} Program`,
  ...
  const selStyle={background:"rgba(10,10,20,0.96)",border:`1px solid ${T.border}`,borderRadius:9,
  ...
  return <div>
    <div style={{display:"grid",gridTemplateColumns:"310px 1fr",gap:16}}>
      <Card style={{padding:22,height:"fit-content",position:"sticky",top:0}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:11}}>
          ...
          <button onClick={generate} disabled={loading}
            style={{background:`linear-gradient(135deg,${T.blue},#2040a0)`,border:"none",borderRadius:10,
      ...
      <div>
        {!result&&!loading&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",
        ...
        {loading&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",
          minHeight:380,flexDirection:"column",gap:16}}>
          <div style={{width:48,height:48,borderRadius:"50%",border:`3px solid ${T.blue}28`,
            borderTop:`3px solid ${T.blue}`,animation:"spin 1s linear infinite"}}/>
        ...
        {result&&<div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(272px,1fr))",gap:12,marginBottom:12}}>
            {(result.days||[]).map((d,i)=>{
              ...
              return <Card key={i} style={{padding:16}}>
                ...
                {(d.exercises||[]).map((ex,j)=>(
                  <div key={j} style={{display:"flex",gap:10,padding:"8px 0",
                    borderBottom:j<(d.exercises||[]).length-1?`1px solid ${T.border}`:"none",alignItems:"flex-start"}}>
                    <div style={{width:20,height:20,borderRadius:5,background:`${fc}18`,border:`1px solid ${fc}26`,
                      display:"flex",alignItems:"center",justifyContent:"center",
                      fontSize:9,color:fc,fontWeight:800,flexShrink:0,marginTop:1}}>{j+1}</div>
